...
bool TorrentExpanderImpl::Collapse() {
  int max_level = MaxLevel();
  Collapse(max_level - 1);
  return MaxLevel() != max_level;
}

int TorrentExpanderImpl::MinLevel() const {
  if (!expanded) {
    return 0;
  }
  if (children_.size() == 0) {
...
int TorrentExpanderImpl::MaxLevel() const {
  if (!expanded) {
    return 0;
  }
  int max_level = 1;
...
void TorrentExpanderImpl::Expand(int minLevel) {
  if (minLevel <= 0)
    return;
  expanded = true;
  minLevel--;
...
void TorrentExpanderImpl::Collapse(int maxLevel) {
  if (maxLevel <= 0) {
    expanded = false;
  }
  maxLevel--;
