...
Component MakeHelpModal(bool *shown)
{
  ...
  auto help_renderer = Renderer([sections, shown](bool /*focused*/)
                  {
    Elements rows;
  ...
  // Add event handling to close the modal
  return CatchEvent(help_renderer, [shown](Event event) {
    if (event == Event::Character('?') || event == Event::Escape || event == Event::Character('q')) {
      *shown = false;
      return true;
    }
    return false;
